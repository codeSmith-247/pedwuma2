...
import { userRank } from "functions/utils/Locations";
...
export function ServiceSearch({ callback = console.log, status="active", ...props }) {
    ...
    return (
        <Search
            ...
            searchCallback={(data, setInput) => {setInput(data?.Title)}}
            searchingCallback={setSearchInput}
            resultUi={ (item) =>
                <div>
                    {safeGet(item, ["Services Provided"], []).map( (sub, subIndex) => 
                        <div key={subIndex} onClick={() => callback(sub)} className="px-5 py-3 text-md hover:bg-gray-200 active:bg-blue-600 active:text-white">
                            {safeGet(item, ["Category Name"], "")}-({safeGet(sub, ["Title"], "")})
                        </div>
                    )}
                </div>
            }
...
export default function() {
    ...
    console.log(userRank(safeGet(data, ["0"], [])));
    ...
    return (
        <div className="p-10 max-[1165px]:p-0 h-[700px] max-[1165px]:h-max max-[1165px]:flex-col max-[1165px]:gap-0 flex gap-5 z-10 relative">
            <form className="bg-white rounded-md h-max max-w-[570px] max-[1165px]:max-w-[100%] max-[1165px]:rounded-none shadow sticky">
                ...
                <div className="p-10 max-[475px]:p-5">
                    ...
                    <ServiceSearch  callback={(sub) => {console.log(sub); setParameters({...parameters, service: safeGet(sub, ["Title"], "")}) }} sx={{marginBottom: "2rem"}}/>
        ...
        { searchData !== null &&
            <div data-aos="fade-in" className="flex-grow bg-black bg-opacity-30 min-h-[500px] max-[1165px]:backdrop-blur-xl rounded overflow-y-scroll relative">
                {data && userRank(safeGet(data, ["0"], [])).length <= 0 && 
                ...
                {userRank(safeGet(data, ["0"], [])).map( (item, index) => 
                    <div onClick={() => navigate(`/worker/${encrypt(item.id)}`)} className="flex gap-3 bg-[#111] text-white hover:bg-[#333] mb-1.5 p-1.5 rounded-md">
